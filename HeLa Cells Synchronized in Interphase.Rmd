...
The data set **Sel_Crit_PCA** (which will be used for PCA) contains the following information for each gene:
  ...
  * **shift**: character variable that states whether a protein is left, right or non shifting (shifting defined as delta mu over 1 fraction) (not used directly in PCA but for visualizing the results)
...
# ---> "shift": Kategorisieren der Gene in "left", "right" and "no" shifting anhand der Mittelwerte der gefitteten Gaußkurven (wir gehen erst von einem Shift aus, wenn die Differenz der Mittelwerte größer als 1 Fraktion ist)
Sel_Crit_PCA[, 8] = rep(NA, nrow(Ctrl_RDeep))
Sel_Crit_PCA[which((Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) > 1), 8] = "left"
Sel_Crit_PCA[which((Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) < -1), 8] = "right"
Sel_Crit_PCA[which((Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) <= 1 & (Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) >= -1), 8] = "no"
