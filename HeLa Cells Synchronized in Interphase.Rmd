...
##### 4.2 Dataframe with all three selection criteria in comparison 

In order to determine which proteins are significantly RNA-dependent, we will apply three selection criteria comparing different characteristics of the Gaussian curves fitted to the Ctrl and RNase global maxima:

  *Shift of global maximum/mu by at least 1 bin
  *The height of the global maximum should be at least twice as large as the height of the respective other curve at the same position (and if latter is larger than former, the sample is discarded completely)
  *Overlap of the curves of less than 75% 


```{r}
#First, a dataframe is created which includes all parameters we want to test for in the next step.

Sel_Crit_compare = as.data.frame(matrix(rep(NA, nrow(Sel_Crit)*6), ncol = 6))
colnames(Sel_Crit_compare) = c("delta_mu", "Ctrl_heightprop", "RNase_heightprop", "overlap", "Ctrl_heightprop_compare", "RNase_heightprop_compare")
rownames(Sel_Crit_compare) = rownames(Sel_Crit)

Sel_Crit_compare[,1] <- round(abs(Sel_Crit[,3] - Sel_Crit[,7]), digits = 3) 

Sel_Crit_compare[,2] <- round(Sel_Crit[,5]/(Sel_Crit[,9]*(1/(Sel_Crit[,8]*(sqrt(2*pi))))*exp(1)^(-(1/2)*((Sel_Crit[,3]-Sel_Crit[,7])/Sel_Crit[,8])^2)), digits = 3)

Sel_Crit_compare[,3] <- round(Sel_Crit[,9]/(Sel_Crit[,5]*(1/(Sel_Crit[,4]*(sqrt(2*pi))))*exp(1)^(-(1/2)*((Sel_Crit[,7])-Sel_Crit[,3]/Sel_Crit[,4])^2)), digits = 3)

Sel_Crit_compare[,4] <- round(Sel_Crit[,11], digits = 3)

Sel_Crit_compare[,5] <- round(Ctrl_Mean_Smooth_Norm100[,Mean_Gbl_Max[,1]]/RNase_Mean_Smooth_Norm100[,Mean_Gbl_Max[,1]], digits = 3)

Sel_Crit_compare[,6] <- round(RNase_Mean_Smooth_Norm100[,Mean_Gbl_Max[,2]]/Ctrl_Mean_Smooth_Norm100[,Mean_Gbl_Max[,2]], digits = 3)

```

The proteins where the height proportion is smaller than 1 are discarded completely.

```{r}
#Delete rows where at least one of the height proportions is smaller than one 

which(Sel_Crit_compare[,2] < 1)
#No samples need to be deleted
which(Sel_Crit_compare[,3] < 1)
#No samples need to be deleted

```

Now we will create a copy of the dataframe and remove all proteins which do not satisfy the selection criteria, and add these to a new dataframe (i.e. non-RDPs), to be left with a dataframe containing all RDPs evaluated by our analysis. 

```{r}
#Delete rows where the selection criteria do not apply in the "RDPs"-dataframe, and add these rows to a "non-RDPs"-dataframe to be left with a dataframe containing all RDPs, and another dataframe with all non-RDPs.

RDPs <- Sel_Crit_compare
non_RDPs <- Sel_Crit_compare

#Criterion 1: mu >= 1 

non_RDPs <- non_RDPs[-which(RDPs[,1] >= 1),]
RDPs <- RDPs[which(RDPs[,1] >= 1),]

#Criterion 2: height proportion >= 2

non_RDPs <- rbind(non_RDPs, RDPs[-which(RDPs[,2] >= 2),])
RDPs <- RDPs[which(RDPs[,2] >= 2),]

non_RDPs <- rbind(non_RDPs, RDPs[-which(RDPs[,3] >= 2),])
RDPs <- RDPs[which(RDPs[,3] >= 2),]

#Criterion 3: overlap <= 0.75

non_RDPs <- rbind(non_RDPs, RDPs[-which(RDPs[,4] <= 0.75),])
RDPs <- RDPs[which(RDPs[,4] <= 0.75),]
...
plot(Sel_Crit[,3], Sel_Crit[,7], main = "Shifting behavior", xlab = "Peak of Ctrl", ylab = "Peak of RNase")


shift_behavior <- cbind(Sel_Crit[,3], Sel_Crit[,7])

shift_behavior = cbind(shift_behavior, kmeans(shift_behavior, 2, iter.max = 10, nstart = 1)$cluster)

shift_behavior[which(shift_behavior[,3] == 1), 3] = "yellow"
shift_behavior[which(shift_behavior[,3] == 2), 3] = "red"

plot(Sel_Crit[,3], Sel_Crit[,7], main = "Shifting behavior", xlab = "Peak of Ctrl", ylab = "Peak of RNase", col = c(shift_behavior[,3]))


shift_behavior <- cbind(Sel_Crit[,3], Sel_Crit[,7])

shift_behavior = cbind(shift_behavior, kmeans(shift_behavior, 3, iter.max = 10, nstart = 1)$cluster)

shift_behavior[which(shift_behavior[,3] == 1), 3] = "yellow"
shift_behavior[which(shift_behavior[,3] == 2), 3] = "red"
shift_behavior[which(shift_behavior[,3] == 3), 3] = "blue"
...
shift_behavior = cbind(shift_behavior, kmeans(shift_behavior, 5, iter.max = 10, nstart = 1)$cluster)

shift_behavior[which(shift_behavior[,3] == 1), 3] = "yellow"
shift_behavior[which(shift_behavior[,3] == 2), 3] = "red"
shift_behavior[which(shift_behavior[,3] == 3), 3] = "blue"
shift_behavior[which(shift_behavior[,3] == 4), 3] = "green"
shift_behavior[which(shift_behavior[,3] == 5), 3] = "magenta"
...
shift_behavior = cbind(shift_behavior, kmeans(shift_behavior, 10, iter.max = 10, nstart = 1)$cluster)
...
shift_behavior[which(shift_behavior[,3] == 4), 3] = "green"
shift_behavior[which(shift_behavior[,3] == 5), 3] = "magenta"
shift_behavior[which(shift_behavior[,3] == 6), 3] = "orange"
shift_behavior[which(shift_behavior[,3] == 7), 3] = "pink"
shift_behavior[which(shift_behavior[,3] == 8), 3] = "darkgreen"
shift_behavior[which(shift_behavior[,3] == 9), 3] = "black"
shift_behavior[which(shift_behavior[,3] == 10), 3] = "darkred"

plot(Sel_Crit[,3], Sel_Crit[,7], main = "Shifting behavior", xlab = "Peak of Ctrl", ylab = "Peak of RNase", col = c(shift_behavior[,3]))


plot(Sel_Crit_compare[,1], Sel_Crit_compare[,4])

shift_behavior <- cbind(Sel_Crit[,1], Sel_Crit[,4])
...
plot(Sel_Crit_compare[,1], Sel_Crit_compare[,4], col = c(shift_behavior[,3]))
```

##### 5.2 Principal Component Analysis (& k-Means clustering)

We want to determine how good the selection criteria are that we have chosen. To do this we will use **Principal Component Analysis (PCA)**. The main idea is that we will use PCA to reduce the dimensions of a data set containing our selection criteria and additional information about the global peaks. If our selection criteria are sufficient to determine the RNA-dependency of a protein the points in our reduced data set should separate nicely along these criteria. 

The data set **Sel_Crit_PCA** (which will be used for PCA) contains the following information for each gene:

  * **delta_mu [%]**: percentage of peak shift between the Ctrl and RNase peaks (absolute shift/25)
  * **delta_sd [%]**: percentage of change in the standard deviation between Ctrl and RNase (sd2/sd1 - 1)
  * **delta_h [%]**: percentage of change in peak height between Ctrl and RNase (h2/h1 - 1)
  * **Ctrl_h_prop [%]**: how much percent of protein amount under the Ctrl peak is lost upon RNase treatment? (prt2/prt1 - 1)
  * **RNase_h_prop [%]**: how much percent of protein amount under the RNase peak is gained upon RNase treatment? (1 - prt1/prt2)
  * **delta_prt_amount [%]**: percentage of change between the protein amount under the Ctrl and RNase peaks (peak_prt2/peak_prt1 - 1)
  * **overlap [%]**: percentage of overlap between the gaussian curves of Ctrl and RNase 
  * **shift**: character variable that states whether a protein is **left**, **right** or **non** shifting (shifting defined as delta mu over 1 fraction) or **precipitated** (if one of the global peaks are over 24) (not used directly in PCA but for visualizing the results)
  
**Note**: *All variables are calculated as percentages so that they are comparable to each other.*

```{r}
# ---> Erstellen eines leeren Dataframes (nur eine Spalte NA -> weiter Spalten können angefügt werden)
Sel_Crit_PCA = as.data.frame(rep(NA, nrow(Ctrl_RDeep)))

# ---> "delta_mu [%]": Berechnung des prozentualen Peak Shifts über alle Gene
Sel_Crit_PCA[, 1] = (Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1])/25

# ---> "delta_sd [%]": Berechnung der prozentualen Veränderung der Standardabweichung zwischen Ctrl und RNase für alle Gene
Sel_Crit_PCA[, 2] = RNase_Gauss_Fit[, 2]/Ctrl_Gauss_Fit[, 2] - 1

# ---> "delta_h [%]": Berechnung der prozentualen Veränderung der Höhe zwischen Ctrl und RNase für alle Gene
Sel_Crit_PCA[, 3] = RNase_Gauss_Fit[, 3]/Ctrl_Gauss_Fit[, 3] - 1


# ---> "Ctrl_h_prop [%]": 1. Berechnung der Fraktions-Werte für die gilt, dass der y-Wert der Gausskurve für Ctrl bei 2 liegt 
x1 = Ctrl_Gauss_Fit[, 2] * sqrt(-2 * log((2*sqrt(2*pi)*Ctrl_Gauss_Fit[, 2])/(Ctrl_Gauss_Fit[, 3]))) + Ctrl_Gauss_Fit[, 1]
x2 = - Ctrl_Gauss_Fit[, 2] * sqrt(-2 * log((2*sqrt(2*pi)*Ctrl_Gauss_Fit[, 2])/(Ctrl_Gauss_Fit[, 3]))) + Ctrl_Gauss_Fit[, 1]

# ---> "Ctrl_h_prop [%]": 2. Ersetzen aller Werte über 25 mit 25 und unter 1 mit 1(Fraktion 0 existiert nicht im Datensatz)
x1[which(x1 > 25)] = 25
x2[which(x2 < 1)] = 1 

# ---> "Ctrl_h_prop [%]": 3. Runden der erhaltenen Fraktions-Werte auf ganze Zahlen (Note: x1 > x2 für alle Gene)
x1 = ceiling(x1)
x2 = floor(x2)

# ---> "Ctrl_h_prop [%]": 4. Erstellen einer Sequenz für jedes Gen, die von x2 bis x1 - 1 geht (für die Berechnung der Fläche unter der Kurve nur anhand der Daten aus "Ctrl_Mean_Smooth_Norm100" bzw. "RNase_Mean_Smooth_Norm100")
Fra.Pos = sapply(seq(1, nrow(Ctrl_RDeep), 1), function(n) {seq(x2[n], x1[n] - 1, 1)})

# ---> "Ctrl_h_prop [%]": 5. Berechnung des Protein-Anteils unter der geglätteten und normalisierten Protein-Verteilung von Ctrl anhand von "Fra.Pos" (mathematische Flächenberechnung von Trapezen)
Ctrl_prt_amount = sapply(seq(1, nrow(Ctrl_RDeep), 1), function(n) {sum(unlist(sapply(seq(1, length(Fra.Pos[[n]]), 1), function(i) {Ctrl_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i]] + 1/2 * (Ctrl_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i + 1]] - Ctrl_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i]])})))})

# ---> "Ctrl_h_prop [%]": 6. Gleiche Berechnung für RNase (Note: hier an der Position des globalen Maximums der Ctrl-Verteilung)
RNase_prt_amount = sapply(seq(1, nrow(RNase_RDeep), 1), function(n) {sum(unlist(sapply(seq(1, length(Fra.Pos[[n]]), 1), function(i) {RNase_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i]] + 1/2 * (RNase_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i + 1]] - RNase_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i]])})))})

# ---> "Ctrl_h_prop [%]": 7. Berechnung, um wie viel Prozent der Protein-Anteil unter dem globalen Maximums der Ctrl abnimmt nach Zugabe von RNase
Sel_Crit_PCA[, 4] = RNase_prt_amount/Ctrl_prt_amount - 1


# ---> "RNase_h_prop [%]": Gleiche Berechnungen wie für "Ctrl_h_prop [%]" für RNase (Note: hier wird aber berechnet, um wie viel Prozent der Protein-Anteil unter dem globalen Maximum der RNase *zunimmt* nach RNase-Zugabe)
x1 = RNase_Gauss_Fit[, 2] * sqrt(-2 * log((2*sqrt(2*pi)*RNase_Gauss_Fit[, 2])/(RNase_Gauss_Fit[, 3]))) + RNase_Gauss_Fit[, 1]
x2 = - RNase_Gauss_Fit[, 2] * sqrt(-2 * log((2*sqrt(2*pi)*RNase_Gauss_Fit[, 2])/(RNase_Gauss_Fit[, 3]))) + RNase_Gauss_Fit[, 1]
...
Fra.Pos = sapply(seq(1, nrow(RNase_RDeep), 1), function(n) {seq(x2[n], x1[n] - 1, 1)})

Ctrl_prt_amount = sapply(seq(1, nrow(Ctrl_RDeep), 1), function(n) {sum(unlist(sapply(seq(1, length(Fra.Pos[[n]]), 1), function(i) {Ctrl_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i]] + 1/2 * (Ctrl_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i + 1]] - Ctrl_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i]])})))})

RNase_prt_amount = sapply(seq(1, nrow(RNase_RDeep), 1), function(n) {sum(unlist(sapply(seq(1, length(Fra.Pos[[n]]), 1), function(i) {RNase_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i]] + 1/2 * (RNase_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i + 1]] - RNase_Mean_Smooth_Norm100[n, Fra.Pos[[n]][i]])})))})

Sel_Crit_PCA[, 5] = 1 - Ctrl_prt_amount/RNase_prt_amount


# ---> "delta_prt_amount [%]": Berechnung der prozentualen Veränderung des Protein-Anteils unter den Gaußkurven zwischen Ctrl und RNase
Sel_Crit_PCA[, 6] = RNase_Gauss_Fit[, 4]/Ctrl_Gauss_Fit[, 4] - 1

# ---> "overlap [%]": Hinzufügen der prozentualen Überlappung der Gaußkurven von Ctrl und RNase (Berechnung: s. "3.4 Calculation of Gaussian Overlap")
Sel_Crit_PCA[, 7] = Ctrl_Gauss_Fit[, 5]

# ---> "shift": Kategorisieren der Gene in "left", "right" and "no" shifting und "precipitated" anhand der Mittelwerte der gefitteten Gaußkurven (wir gehen erst von einem Shift aus, wenn die Differenz der Mittelwerte größer als 1 Fraktion ist)
Sel_Crit_PCA[, 8] = rep(NA, nrow(Ctrl_RDeep))
Sel_Crit_PCA[which((Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) > 1 & !(Ctrl_Gauss_Fit[, 1] > 24 | RNase_Gauss_Fit[, 1] > 24)), 8] = "left"
Sel_Crit_PCA[which((Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) < -1 & !(Ctrl_Gauss_Fit[, 1] > 24 | RNase_Gauss_Fit[, 1] > 24)), 8] = "right"
Sel_Crit_PCA[which(Ctrl_Gauss_Fit[, 1] > 24 | RNase_Gauss_Fit[, 1] > 24), 8] = "precipitated"
Sel_Crit_PCA[which((Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) <= 1 & (Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) >= -1 & !(Ctrl_Gauss_Fit[, 1] > 24 | RNase_Gauss_Fit[, 1] > 24)), 8] = "no"

# ---> Benennung der Zeilen und Spalten des Dataframes
rownames(Sel_Crit_PCA) = rownames(Sel_Crit)
colnames(Sel_Crit_PCA) = c("delta_mu [%]", "delta_sd [%]", "delta_h [%]", "Ctrl_h_prop [%]", "RNase_h_prop [%]", "delta_prt_amount [%]", "overlap [%]", "shift")
...
rm(Fra.Pos, x1, x2, Ctrl_prt_amount, RNase_prt_amount, col)
```

We will now use PCA to determine new (non-correlating) variables (**Principal Components** (**PC**)) for our dataframe:

```{r}
# ---> PCA mit dem "prcomp" Befehl
pca = prcomp(Sel_Crit_PCA[, 1:7])

# ---> "$sdev" gibt die Standardabweichungen der PCs aus, also erhalten wir durch Quadrieren die Varianz der einzelnen PCs und, wenn wir diese durch die Gesamtvarianz teilen, den prozentualen Anteil der einzelnen PCs an der Gesamtvarianz des Datensatzes "Sel_Crit_PCA"
...
# ---> Plot, der die Datenpunkte je nach Kategorisierung in "RDP" oder "non RDP" einfärbt (nimmt Einteilung aus den Berechnungen aus "4.2 Dataframe with all three selection criteria in comparison")
col = rep(NA, nrow(Ctrl_RDeep))
col[which(rownames(Ctrl_Gauss_Fit) %in% rownames(RDPs))] = "blue"
col[which(rownames(Ctrl_Gauss_Fit) %in% rownames(non_RDPs))] = "grey"

plot(pca$x[, 1], pca$x[, 2], pch = 16, col = col, main = "RDPs and non RDPs", xlab = "PC1", ylab = "PC2")
legend("topright", legend = c("RDP", "non RDP"), pch = 16, col = c("blue", "grey"))
...
fviz_pca_biplot(pca, axes = c(1,2) , label = "var", col.var = "black", repel = T, col.ind = Sel_Crit_PCA[,8])
...
legend("topright", legend = c("1", "2", "3"), pch = 16, col = c("blue", "red", "orange"))
...
We can also repeat the PCA but this time we will only apply it to our selection criteria **delta_mu [%]**, **Ctrl_h_prop [%]**, **RNase_h_prop [%]** and **overlap [%]**: 

```{r}
# ---> PCA
pca = prcomp(Sel_Crit_PCA[, c(1, 4, 5, 7)])
...
legend("topright", legend = c("1", "2"), pch = 16, col = c("blue", "red"))
...
##### 5.4 Comparison with the RBP2GO Database
...
Next, we expand our dataframe **Sel_Crit_PCA** with a ninth column containing the RBP2GO-Score for each gene: 

```{r}
Sel_Crit_PCA[, 9] = NA
colnames(Sel_Crit_PCA) = c(colnames(Sel_Crit_PCA)[-9], "RBP2GO_Score")

Sel_Crit_PCA[which(rownames(Sel_Crit_PCA) %in% rownames(Table_HS_RBP)), 9] = Table_HS_RBP[which(rownames(Table_HS_RBP) %in% rownames(Sel_Crit_PCA)), 3]

Sel_Crit_PCA[which(rownames(Sel_Crit_PCA) %in% rownames(Table_HS_Non_RBP)), 9] = Table_HS_Non_RBP[which(rownames(Table_HS_Non_RBP) %in% rownames(Sel_Crit_PCA)), 3]
```
...
col = rep(NA, nrow(Ctrl_RDeep))
col[which(rownames(Ctrl_Gauss_Fit) %in% rownames(Table_HS_RBP))] = "blue"
col[which(rownames(Ctrl_Gauss_Fit) %in% rownames(Table_HS_Non_RBP))] = "grey"
...
###### 5.4.2 Comparison with RBP2GO-Score
...
col = rep(NA, nrow(Sel_Crit_PCA))
col[which(is.na(Sel_Crit_PCA[, 9]))] = "grey" # 20
col[which(Sel_Crit_PCA[, 9] <= 0.9)] = "purple" # 1039
col[which(0.9 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 2.3)] = "blue" # 1075
col[which(2.3 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 4)] = "green" # 993
col[which(4 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 6.9)] = "yellow" # 973
col[which(6.9 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 15.9)] = "orange" # 995
col[which(15.9 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 100)] = "red" # 895
...
plot(pca$x[, 1], pca$x[, 2], pch = 16, col = alpha("blue", Sel_Crit_PCA[, 9]/100), main = "RBP2GO_Score", xlab = "PC1", ylab = "PC2")
...
###### 5.4.3 Cluster Comparison with t-Test
...
plot(pca$x[, 1], pca$x[, 2], pch = 16, col = col, main = "k-Means Clustering (n = 3)", xlab = "PC1", ylab = "PC2")
par(xpd=T)
legend("topright", inset = c(-0.5,0), legend = c(paste("Cluster 1 (n = ", length(which(kmeans$cluster == 1)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit_PCA[which(kmeans$cluster == 1), 9], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit_PCA[which(kmeans$cluster == 1), 9], na.rm = T)/sqrt(length(which(kmeans$cluster == 1))), 2), ")", sep = ""), paste("Cluster 2 (n = ", length(which(kmeans$cluster == 2)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit_PCA[which(kmeans$cluster == 2), 9], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit_PCA[which(kmeans$cluster == 2), 9], na.rm = T)/sqrt(length(which(kmeans$cluster == 2))), 2), ")", sep = ""), paste("Cluster 3 (n = ", length(which(kmeans$cluster == 3)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit_PCA[which(kmeans$cluster == 3), 9], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit_PCA[which(kmeans$cluster == 3), 9], na.rm = T)/sqrt(length(which(kmeans$cluster == 3))), 2), ")", sep = ""), "", paste("p(1&2) = ", signif(t.test(Sel_Crit_PCA[which(kmeans$cluster == 1), 9], Sel_Crit_PCA[which(kmeans$cluster == 2), 9], alternative = "two.sided")$p.value, 3), sep = ""), paste("p(1&3) = ", signif(t.test(Sel_Crit_PCA[which(kmeans$cluster == 1), 9], Sel_Crit_PCA[which(kmeans$cluster == 3), 9], alternative = "two.sided")$p.value, 3), sep = ""), paste("p(2&3) = ", signif(t.test(Sel_Crit_PCA[which(kmeans$cluster == 2), 9], Sel_Crit_PCA[which(kmeans$cluster == 3), 9], alternative = "two.sided")$p.value, 3), sep = "")), pch = 16, col = c("blue", NA, "red", NA, "orange", NA, NA, NA, NA, NA))
...
legend("topright", 
       inset = c(-0.475,0), 
       legend = c(paste("Cluster 1 (n = ", length(which(kmeans$cluster == 1)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit_PCA[which(kmeans$cluster == 1), 9], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit_PCA[which(kmeans$cluster == 1), 9], na.rm = T)/sqrt(length(which(kmeans$cluster == 1))), 2), ")", sep = ""), paste("Cluster 2 (n = ", length(which(kmeans$cluster == 2)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit_PCA[which(kmeans$cluster == 2), 9], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit_PCA[which(kmeans$cluster == 2), 9], na.rm = T)/sqrt(length(which(kmeans$cluster == 2))), 2), ")", sep = ""), "", paste("p(1&2) = ", signif(t.test(Sel_Crit_PCA[which(kmeans$cluster == 1), 9], Sel_Crit_PCA[which(kmeans$cluster == 2), 9], alternative = "two.sided")$p.value, 3), sep = "")), 
       pch = 16, 
...
draw.triple.venn(
  area1 = nrow(RDPs),   # Our identified RDPs
  ...
  n12 = length(which(rownames(Table_HS_RBP) %in% rownames(RDPs))), 
  n23 = length(which(Table_HS_RBP$R.DeeP_HeLa == "X")),
  n13 = length(which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(RDPs))), 
  n123 = length(which(which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(RDPs)) %in% which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(Table_HS_RBP)))),
  fill = c("dodgerblue", "darkorange", "red"),
  lwd = 1,
  category = c(paste("Our RDPs (", nrow(RDPs), ")", sep = ""), paste("RBP2GO RBPs (", nrow(Table_HS_RBP), ")", sep =""), paste("R-Deep (", length(which(Table_HS_RBP$R.DeeP_HeLa == "X")), ")", sep = "")),
  fontfamily = "plain",
...
draw.triple.venn(
  area1 = nrow(non_RDPs),   # Our identified non RDPs
  area2 = nrow(Table_HS_RBP),  # RBPs in RBP2GO-Databank
  area3 = length(which(Table_HS_RBP$R.DeeP_HeLa == "X")),   # RDPs from the R-Deep-Databank
  n12 = length(which(rownames(Table_HS_RBP) %in% rownames(non_RDPs))), 
  n23 = length(which(Table_HS_RBP$R.DeeP_HeLa == "X")),
  n13 = length(which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(non_RDPs))), 
  n123 = length(which(which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(non_RDPs)) %in% which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(Table_HS_RBP)))),
  fill = c("dodgerblue", "darkorange", "red"),
  lwd = 1,
  category = c(paste("Our non RDPs (", nrow(non_RDPs), ")", sep = ""), paste("RBP2GO RBPs (", nrow(Table_HS_RBP), ")", sep =""), paste("R-Deep (", length(which(Table_HS_RBP$R.DeeP_HeLa == "X")), ")", sep = "")),
  fontfamily = "plain",
  cat.fontfamily = "plain",
  cat.fontface = "bold",
  cat.dist = c(0.09, 0.085, 0.08),
...
par(mar=c(1, 3, 1, 7.5))
barplot(matrix(c(
  length(which(rownames(non_RDPs) %in% rownames(Table_HS_Non_RBP))) + length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(non_RDPs))),
  length(which(rownames(non_RDPs) %in% rownames(Table_HS_RBP))),
  length(which(rownames(RDPs) %in% rownames(Table_HS_RBP))),
  length(which(rownames(RDPs) %in% rownames(Table_HS_Non_RBP))) + length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(RDPs))),
  rep(NA, 4)),
  nrow = 4),
  axes = F,
  col = c("red", "orange", "yellow", "green"), 
  legend.text = c(paste("Non RDPs (", length(which(rownames(non_RDPs) %in% rownames(Table_HS_Non_RBP))) + length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(non_RDPs))), ")", sep = ""), 
                  paste("RDPs (only RBP2GO) (", length(which(rownames(non_RDPs) %in% rownames(Table_HS_RBP))), ")", sep = ""), 
                  paste("RDPS (", length(which(rownames(RDPs) %in% rownames(Table_HS_RBP))), ")", sep = ""), 
                  paste("New RDPs (", length(which(rownames(RDPs) %in% rownames(Table_HS_Non_RBP))) + length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(RDPs))), ")", sep = "")),
  )
```
...
```{r gene plotter}
Gene_Plotter(c("HNRPL_HUMAN", "CTCF_HUMAN"))
```
