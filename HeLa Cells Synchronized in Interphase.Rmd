...
Fra.Pos = sapply(seq(1, nrow(Ctrl_RDeep), 1), function(i) {which(Ctrl_Mean_Smooth_Norm100[i,] == max(Ctrl_Mean_Smooth_Norm100[i,]))})
Fra.Val = sapply(seq(1, nrow(Ctrl_RDeep), 1), function(n) {RNase_Mean_Smooth_Norm100[n, Fra.Pos[[n]]]})
Fra.Val = sapply(seq(1, nrow(Ctrl_RDeep), 1), function(n) {mean(unlist(Fra.Val[[n]]))})

Sel_Crit_PCA[, 4] = Fra.Val/apply(Ctrl_Mean_Smooth_Norm100, 1, function(x) {max(x)}) - 1


Fra.Pos = sapply(seq(1, nrow(Ctrl_RDeep), 1), function(i) {which(RNase_Mean_Smooth_Norm100[i,] == max(RNase_Mean_Smooth_Norm100[i,]))})
Fra.Val = sapply(seq(1, nrow(Ctrl_RDeep), 1), function(n) {Ctrl_Mean_Smooth_Norm100[n, Fra.Pos[[n]]]})
Fra.Val = sapply(seq(1, nrow(Ctrl_RDeep), 1), function(n) {mean(unlist(Fra.Val[[n]]))})

Sel_Crit_PCA[, 5] = 1 - Fra.Val/apply(RNase_Mean_Smooth_Norm100, 1, function(x) {max(x)})
rm(Fra.Pos, Fra.Val)
...
colnames(Sel_Crit_PCA) = c("delta mu [%]", "delta sd [%]", "delta h [%]", "Ctrl_h_prop [%]", "RNase_h_prop [%]", "delta prt_amount [%]", "overlap [%]", "shift")
...
plot(pca$x[, 1], pca$x[, 2], pch = 16, col = col, main = "Shifting Behaviour", xlab = "PC1", ylab = "PC2")
...
Gene_Plotter <- function(gene.name, col = c("green", "red", "dodgerblue", "darkorange"), Ctrl = T, RNase = T, Gbl.Max = T, Loc.Max = T, Gauss = T, grid = T, Type = "Smooth and Norm" #, "Norm", "Smooth", "Raw", "Compare"
                          ) {
library(scales)
