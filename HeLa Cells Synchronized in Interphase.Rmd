...
Next, we will compare our selection of RDPs with the RBP2GO databank: 
...
col[which(is.na(Sel_Crit_PCA[, 9]))] = "grey"
col[which(Sel_Crit_PCA[, 9] <= 1)] = "purple"
col[which(1 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 2)] = "blue"
col[which(2 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 3)] = "cyan"
col[which(3 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 4)] = "green"
col[which(4 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 5)] = "yellow"
col[which(5 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 10)] = "orange"
col[which(10 < Sel_Crit_PCA[, 9] & Sel_Crit_PCA[, 9] <= 100)] = "red"
...
mean(Sel_Crit_PCA[which(kmeans$cluster == 1), 9], na.rm = T)
sd(Sel_Crit_PCA[which(kmeans$cluster == 1), 9], na.rm = T)/sqrt(length(which(kmeans$cluster == 1)))
mean(Sel_Crit_PCA[which(kmeans$cluster == 2), 9], na.rm = T)
sd(Sel_Crit_PCA[which(kmeans$cluster == 2), 9], na.rm = T)/sqrt(length(which(kmeans$cluster == 2)))
mean(Sel_Crit_PCA[which(kmeans$cluster == 3), 9], na.rm = T)
sd(Sel_Crit_PCA[which(kmeans$cluster == 3), 9], na.rm = T)/sqrt(length(which(kmeans$cluster == 3)))
...
if(conf == T & Ctrl == T) {
polygon(c(seq(1,25,1), seq(25,1,-1)), c(Ctrl_Mean_Smooth_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),] - Ctrl_Norm100_sd[which(rownames(Ctrl_Mean)==gene.name[j]),], rev(Ctrl_Mean_Smooth_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),] + Ctrl_Norm100_sd[which(rownames(Ctrl_Mean)==gene.name[j]),])), border = NA, col = alpha("grey", 0.6))}

if(conf == T & RNase == T) {
polygon(c(seq(1,25,1), seq(25,1,-1)), c(RNase_Mean_Smooth_Norm100[which(rownames(RNase_Mean)==gene.name[j]),] - RNase_Norm100_sd[which(rownames(RNase_Mean)==gene.name[j]),], rev(RNase_Mean_Smooth_Norm100[which(rownames(RNase_Mean)==gene.name[j]),] + RNase_Norm100_sd[which(rownames(RNase_Mean)==gene.name[j]),])), border = NA, col = alpha("grey", 0.6))}
