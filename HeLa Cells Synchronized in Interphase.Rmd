...
###### 5.2.4 RBP2GO-Score: Linear Regression

Using linear regression we test how well our selection criteria can predict the RBP2GO score:

```{r}
fit <- lm(Sel_Crit[, 27] ~ Sel_Crit[, 1] + Sel_Crit[, 13] + Sel_Crit[, 9] + Sel_Crit[, 11])

summary(fit)$r.squared
```

Seems like not very well :(

###### 5.2.5 RBP2GO-Score: t-Test
...
Gene_Plotter <- function(gene.name, col = c("green", "red", "dodgerblue", "darkorange"), Ctrl = T, RNase = T, Gbl.Max = T, Loc.Max = T, Gauss = T, conf = T, Legend = T, grid = T, Type = "Smooth and Norm" #, "Norm", "Smooth", "Raw", "Compare"
                          ) {
library(scales)
...
Gene_Plotter("CTCF_HUMAN", Type = c("Smooth and Norm", "Norm", "Smooth", "Raw", "Compare"), Legend = F)
```
