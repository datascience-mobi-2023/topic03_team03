...
specifity = 2 * sqrt(2 * log(2)) * Ctrl_Gauss_Fit[which(Sel_Crit[, 26] == "yes"), 2]
...
plot(affinity, specifity, col = col, pch = 16, ylab = "Binding Specificity [Fractions]", xlab = "Binding Affinity [Normalized Protein Amount]")
...
specifity = 2 * sqrt(2 * log(2)) * Ctrl_Gauss_Fit[, 2]
...
curve(fit$coefficients[1] + fit$coefficients[2]*x, add = T, lwd = 3, col = "red")
par(xpd = T)
legend("topright", inset = c(-0.4, 0), legend = c("RDP", "partial RDP", "non RDP", "Regression line", paste("y = ", round(fit$coefficients[2], 2), "x + ", round(fit$coefficients[1], 2), sep = ""), paste("R² = ", round(summary(fit)$r.squared, 2), sep = "")), pch = c(16, 16, 16, NA, NA, NA), col = c("blue", "orange", "grey", "red", NA, NA), lwd = c(NA, NA, NA, 3, NA, NA))
...
```{r gene plotter}
Gene_Plotter("CTCF_HUMAN", Type = c("Smooth and Norm", "Norm", "Smooth", "Compare", "Raw"))
```
