...
We will now summarize all our collected data (*global* and *local peak positions*, *gaussian fit* and *overlap*) in one dataframe called  "**Summary**": 

```{r}
Summary = as.data.frame(cbind(
  Mean_Gbl_Max,   # ---> Global peak positions for Ctrl and RNase
  ...
  Ctrl_Gauss_Fit[, 1],   # ---> Gaussian Fitting Ctrl: mu
  RNase_Gauss_Fit[, 1],   # ---> Gaussian Fitting RNase: mu
  Ctrl_Gauss_Fit[, 2],   # ---> Gaussian Fitting Ctrl: sd
  RNase_Gauss_Fit[, 2],   # ---> Gaussian Fitting RNase: sd
  Ctrl_Gauss_Fit[, 3],   # ---> Gaussian Fitting Ctrl: h
  RNase_Gauss_Fit[, 3],   # ---> Gaussian Fitting RNase: h
  Ctrl_Gauss_Fit[, 4],   # ---> Gaussian Fitting Ctrl: prt_amount
  RNase_Gauss_Fit[, 4],   # ---> Gaussian Fitting RNase: prt_amount
  RNase_Gauss_Fit[, 5]   # ---> Gaussian Fitting: overlap
  ))

colnames(Summary) <- c("Ctrl_Gbl_Max", "RNase_Gbl_Max", "Ctrl_Loc_Max", "RNase_Loc_Max", "Ctrl_mu", "RNase_mu", "Ctrl_sd", "RNase_sd", "Ctrl_h", "RNase_h", "Ctrl_prt_amount", "RNase_prt_amount", "overlap")
...
Based on the variables collected in the dataframe **Summary** we can now calculate the absolute and relative changes between the Ctrl and RNase samples for each gene. The resulting variables are stored in the dataframe "**Sel_Crit**" (for *"Selection Criteria"*).

The variables are the following:

  * **delta_mu**: absolute peak shift between the Ctrl and RNase global peaks (mu1 - mu2)
  * **delta_mu_%**: percentage of peak shift between the Ctrl and RNase global peaks (absolute shift/25)
  
  * **delta_sd**: absolute change in the standard deviation between Ctrl and RNase (sd2 - sd1)
  * **delta_sd_%**: percentage of change in the standard deviation between Ctrl and RNase (sd2/sd1 - 1)
  
  * **delta_h**: absolute change in global peak height between Ctrl and RNase (h2 - h1)
  * **delta_h_%**: percentage of change in peak height between Ctrl and RNase (h2/h1 - 1)
  
  * **delta_prt_amount**: absolute change between the protein amount under the gaussian curves of the Ctrl and RNase peaks (peak_prt2 - peak_prt1)
  * **delta_prt_amount_%**: percentage of change between the protein amount under the gaussian curves of the Ctrl and RNase peaks (peak_prt2/peak_prt1 - 1)
  
  * **Ctrl_prt_prop**: absolute protein amount that is lost under the global peak of Ctrl after RNase treatment (prt1 - prt2)
  * **Ctrl_prt_prop_%**: how much percent of protein amount under the Ctrl peak is lost upon RNase treatment? (1 - prt2/prt1)
  
  * **RNase_prt_prop**: absolute protein amount that is gained under the globak peak of RNase after RNase treatment (prt2 - prt1)
  * **RNase_prt_prop_%**: how much percent of protein amount under the RNase peak is gained upon RNase treatment? (1 - prt1/prt2)
  
  * **overlap_%**: percentage of overlap between the gaussian curves of Ctrl and RNase 
...
**Note**: *"Ctrl_prt_prop"* and *"RNase_prt_prop"* are computed by calculating the x-values (x1 and x2) for which the fitted gaussian curves reach their half maximum y-value (max(y)/2) and then computing the area under the smoothed and normalized protein distributions of Ctrl and RNase in the interval [floor(x2), ceiling(x1)].
...
# ---> "delta_mu": Berechnung des absoluten Peak Shifts über alle Gene
Sel_Crit[, 1] = Summary[, 5] - Summary[, 6]

# ---> "delta_mu_%": Berechnung des prozentualen Peak Shifts über alle Gene
Sel_Crit[, 2] = (Summary[, 5] - Summary[, 6])/25


# ---> "delta_sd": Berechnung der absoluten Veränderung der Standardabweichung zwischen Ctrl und RNase für alle Gene
Sel_Crit[, 3] = Summary[, 8] - Summary[, 7]

# ---> "delta_sd_%": Berechnung der prozentualen Veränderung der Standardabweichung zwischen Ctrl und RNase für alle Gene
Sel_Crit[, 4] = Summary[, 8]/Summary[, 7] - 1


# ---> "delta_h": Berechnung der absoluten Veränderung der Höhe zwischen Ctrl und RNase für alle Gene
Sel_Crit[, 5] = Summary[, 10] - Summary[, 9] 

# ---> "delta_h_%": Berechnung der prozentualen Veränderung der Höhe zwischen Ctrl und RNase für alle Gene
Sel_Crit[, 6] = Summary[, 10]/Summary[, 9] - 1


# ---> "delta_prt_amount": Berechnung der absoluten Veränderung des Protein-Anteils unter den Gaußkurven zwischen Ctrl und RNase
Sel_Crit[, 7] = Summary[, 12] - Summary[, 11]

# ---> "delta_prt_amount_%": Berechnung der prozentualen Veränderung des Protein-Anteils unter den Gaußkurven zwischen Ctrl und RNase
Sel_Crit[, 8] = Summary[, 12]/Summary[, 11] - 1


# ---> "Ctrl_prt_prop": Berechnung des Verlusts an Protein unter dem Ctrl peak nach Zugabe von RNase
# ---> "Ctrl_prt_prop": 1. Berechnung der Fraktions-Werte für die gilt, dass der y-Wert der Gausskurve für Ctrl bei 2 liegt 
x1 = sqrt(- Ctrl_Gauss_Fit[, 2] * log(0.5)) + Ctrl_Gauss_Fit[, 1]
x2 = - sqrt(- Ctrl_Gauss_Fit[, 2] * log(0.5)) + Ctrl_Gauss_Fit[, 1]

# ---> "Ctrl_prt_prop": 2. Ersetzen aller Werte über 25 mit 25 und unter 1 mit 1 (Fraktion 0 existiert nicht im Datensatz)
x1[which(x1 > 25)] = 25
x2[which(x2 < 1)] = 1 

# ---> "Ctrl_prt_prop": 3. Runden der erhaltenen Fraktions-Werte auf ganze Zahlen (Note: x1 > x2 für alle Gene)
x1 = ceiling(x1)
x2 = floor(x2)

# ---> "Ctrl_prt_prop": 4. Erstellen einer Sequenz für jedes Gen, die von x2 bis x1 - 1 geht (für die Berechnung der Fläche unter der Kurve nur anhand der Daten aus "Ctrl_Mean_Smooth_Norm100" bzw. "RNase_Mean_Smooth_Norm100")
Fra.Pos = sapply(seq(1, nrow(Sel_Crit), 1), function(n) {seq(x2[n], x1[n] - 1, 1)})

# ---> "Ctrl_prt_prop": 5. Berechnung des Protein-Anteils unter der geglätteten und normalisierten Protein-Verteilung von Ctrl anhand von "Fra.Pos" (mathematische Flächenberechnung von Trapezen)
...
# ---> "Ctrl_prt_prop": 6. Gleiche Berechnung für RNase (Note: hier an der Position des globalen Maximums der Ctrl-Verteilung)
...
# ---> "Ctrl_prt_prop": 7. Berechnung des absoluten Verlustes an Protein unter dem globalen Maximums der Ctrl Probe nach Zugabe von RNase
Sel_Crit[, 9] = Ctrl_prt_amount - RNase_prt_amount

# ---> "Ctrl_prt_prop_%": 8. Berechnung, um wie viel Prozent der Protein-Anteil unter dem globalen Maximums der Ctrl abnimmt nach Zugabe von RNase
Sel_Crit[, 10] = 1 - RNase_prt_amount/Ctrl_prt_amount


# ---> "RNase_prt_prop" und "RNase_prt_prop_%": Gleiche Berechnungen wie für "Ctrl_prt_prop" und "Ctrl_prt_prop_%" für RNase (Note: hier wird aber berechnet, um wie viel Prozent der Protein-Anteil unter dem globalen Maximum der RNase *zunimmt* nach RNase-Zugabe)
...
# ---> "overlap_%": Hinzufügen der prozentualen Überlappung der Gaußkurven von Ctrl und RNase (Berechnung: s. "3.4 Calculation of Gaussian Overlap")
Sel_Crit[, 13] = Summary[, 13]
...
Sel_Crit[, 14] = rep(NA, nrow(Sel_Crit))
Sel_Crit[which((Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) > 1 & !(Ctrl_Gauss_Fit[, 1] > 24 | RNase_Gauss_Fit[, 1] > 24)), 14] = "left"
Sel_Crit[which((Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) < -1 & !(Ctrl_Gauss_Fit[, 1] > 24 | RNase_Gauss_Fit[, 1] > 24)), 14] = "right"
Sel_Crit[which(Ctrl_Gauss_Fit[, 1] > 24 | RNase_Gauss_Fit[, 1] > 24), 14] = "precipitated"
Sel_Crit[which((Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) <= 1 & (Ctrl_Gauss_Fit[, 1] - RNase_Gauss_Fit[, 1]) >= -1 & !(Ctrl_Gauss_Fit[, 1] > 24 | RNase_Gauss_Fit[, 1] > 24)), 14] = "no"


# ---> Benennung der Zeilen und Spalten des Dataframes
rownames(Sel_Crit) = rownames(Summary)
colnames(Sel_Crit) = c("delta_mu", "delta_mu_%", "delta_sd", "delta_sd_%", "delta_h", "delta_h_%", "delta_prt_amount", "delta_prt_amount_%", "Ctrl_prt_prop", "Ctrl_prt_prop_%", "RNase_prt_prop", "RNase_prt_prop_%", "overlap_%", "shift")

# ---> Entfernung erstellter Variablen
rm(Fra.Pos, x1, x2, Ctrl_prt_amount, RNase_prt_amount)
...
col[which(Sel_Crit[, 14] == "left")] = "blue"
col[which(Sel_Crit[, 14] == "right")] = "red"
col[which(Sel_Crit[, 14] == "precipitated")] = "orange"
col[which(Sel_Crit[, 14] == "no")] = "grey"
...
We can also repeat the PCA but this time we will only apply it to the selection criteria **delta_mu_%**, **Ctrl_prt_prop_%**, **RNase_prt_prop_%** and **overlap_%**: 
...
Based on the PCA we can conclude that the selection criteria **delta_mu**, **Ctrl_prt_prop_%**, **RNase_prt_prop_%** and **overlap** are the most important to separate shifting and non shifting proteins from each other.
...
Furthermore, we define a *partially* RNA-dependent proteins as a protein that satisfies 1. and 4. and either 2. or 3. (1. & 4. & (2. | 3.))

```{r}
# ---> Neue Spalte, die den RDP-Status ("yes", "no" oder "partial") angibt
Sel_Crit[, 15] = rep(NA, nrow(Sel_Crit))
colnames(Sel_Crit) = c(colnames(Sel_Crit)[-15], "RDP")

# ---> Anwenden der Selection Criteria
Sel_Crit[which(abs(Sel_Crit[, 1]) > 1 & Sel_Crit[, 10] > 0.2 & Sel_Crit[, 12] > 0.2 & Sel_Crit[, 13] < 0.75), 15] = "yes"
Sel_Crit[which(!(abs(Sel_Crit[, 1]) > 1 & (Sel_Crit[, 10] > 0.2 | Sel_Crit[, 12] > 0.2) & Sel_Crit[, 13] < 0.75)), 15] = "no"
Sel_Crit[which(abs(Sel_Crit[, 1]) > 1 & (Sel_Crit[, 10] > 0.2 | Sel_Crit[, 12] > 0.2) & Sel_Crit[, 13] < 0.75 & !(Sel_Crit[, 10] > 0.2 & Sel_Crit[, 12] > 0.2)), 15] = "partial"

# ---> Wie viele Gene werden jeweils zugewiesen?
length(which(abs(Sel_Crit[, 1]) > 1 & Sel_Crit[, 10] > 0.2 & Sel_Crit[, 12] > 0.2 & Sel_Crit[, 13] < 0.75))
length(which(!(abs(Sel_Crit[, 1]) > 1 & (Sel_Crit[, 10] > 0.2 | Sel_Crit[, 12] > 0.2) & Sel_Crit[, 13] < 0.75)))
length(which(abs(Sel_Crit[, 1]) > 1 & (Sel_Crit[, 10] > 0.2 | Sel_Crit[, 12] > 0.2) & Sel_Crit[, 13] < 0.75 & !(Sel_Crit[, 10] > 0.2 & Sel_Crit[, 12] > 0.2)))
```


##### 4.6 Visualization of RDPs, non RDPs and partial RDPs

Using the PCA biplot from before we can colorize each gene based on whether or not it is an RDP, non RDP or partial RDP based on our analysis:
...
col = rep(NA, nrow(Sel_Crit))
col[which(Sel_Crit[, 15] == "yes")] = "blue"
col[which(Sel_Crit[, 15] == "partial")] = "orange"
col[which(Sel_Crit[, 15] == "no")] = "grey"
...
legend("topright", legend = c(paste("1 (n = ", length(which(kmeans$cluster == 1)), ")", sep =""), paste("2 (n = ", length(which(kmeans$cluster == 2)), ")", sep =""), paste("3 (n = ", length(which(kmeans$cluster == 3)), ")", sep ="")), pch = 16, col = c("blue", "red", "orange"))
...
legend("topright", legend = c(paste("1 (n = ", length(which(kmeans$cluster == 1)), ")", sep =""), paste("2 (n = ", length(which(kmeans$cluster == 2)), ")", sep ="")), pch = 16, col = c("blue", "red"))
...
Next, we expand our dataframe **Sel_Crit** with a sixteenth column containing the RBP2GO-Score for each gene: 

```{r}
# ---> Erstellen der 16. Spalte (nur NA)
Sel_Crit[, 16] = rep(NA, nrow(Sel_Crit))
colnames(Sel_Crit) = c(colnames(Sel_Crit)[-16], "RBP2GO_Score")

# ---> Hinzufügen der RBP2GO-Scores aus dem Datensatz für RBPs
Sel_Crit[which(rownames(Sel_Crit) %in% rownames(Table_HS_RBP)), 16] = Table_HS_RBP[which(rownames(Table_HS_RBP) %in% rownames(Sel_Crit)), 3]

# ---> Hinzufügen der RBP2GO-Scores aus dem Datensatz für non RBPs
Sel_Crit[which(rownames(Sel_Crit) %in% rownames(Table_HS_Non_RBP)), 16] = Table_HS_Non_RBP[which(rownames(Table_HS_Non_RBP) %in% rownames(Sel_Crit)), 3]
```
...
col = rep(NA, nrow(Sel_Crit))
col[which(is.na(Sel_Crit[, 16]))] = "grey" # 20
col[which(Sel_Crit[, 16] <= 0.9)] = "purple" # 1039
col[which(0.9 < Sel_Crit[, 16] & Sel_Crit[, 16] <= 2.3)] = "blue" # 1075
col[which(2.3 < Sel_Crit[, 16] & Sel_Crit[, 16] <= 4)] = "green" # 993
col[which(4 < Sel_Crit[, 16] & Sel_Crit[, 16] <= 6.9)] = "yellow" # 973
col[which(6.9 < Sel_Crit[, 16] & Sel_Crit[, 16] <= 15.9)] = "orange" # 995
col[which(15.9 < Sel_Crit[, 16] & Sel_Crit[, 16] <= 100)] = "red" # 895
...
plot(pca$x[, 1], pca$x[, 2], pch = 16, col = alpha("blue", Sel_Crit[, 16]/100), main = "RBP2GO_Score", xlab = "PC1", ylab = "PC2")


# ---> Gleiche Berechnungen für die 4 ausgewählten Selektionskriterien
pca = prcomp(Sel_Crit[, c(2, 10, 12, 13)])

plot(pca$x[, 1], pca$x[, 2], pch = 16, col = alpha("blue", Sel_Crit[, 9]/100), main = "RBP2GO_Score", xlab = "PC1", ylab = "PC2")
...
It is interesting that in the second biplot the tail is gone but there are still no clear results :(
...
plot(pca$x[, 1], pca$x[, 2], pch = 16, col = col, main = "k-Means Clustering (n = 3)", xlab = "PC1", ylab = "PC2")
par(xpd=T)
legend("topright", inset = c(-0.5,0), legend = c(paste("Cluster 1 (n = ", length(which(kmeans$cluster == 1)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit[which(kmeans$cluster == 1), 16], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit[which(kmeans$cluster == 1), 16], na.rm = T)/sqrt(length(which(kmeans$cluster == 1))), 2), ")", sep = ""), paste("Cluster 2 (n = ", length(which(kmeans$cluster == 2)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit[which(kmeans$cluster == 2), 16], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit[which(kmeans$cluster == 2), 16], na.rm = T)/sqrt(length(which(kmeans$cluster == 2))), 2), ")", sep = ""), paste("Cluster 3 (n = ", length(which(kmeans$cluster == 3)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit[which(kmeans$cluster == 3), 16], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit[which(kmeans$cluster == 3), 16], na.rm = T)/sqrt(length(which(kmeans$cluster == 3))), 2), ")", sep = ""), "", paste("p(1&2) = ", signif(t.test(Sel_Crit[which(kmeans$cluster == 1), 16], Sel_Crit[which(kmeans$cluster == 2), 16], alternative = "two.sided")$p.value, 3), sep = ""), paste("p(1&3) = ", signif(t.test(Sel_Crit[which(kmeans$cluster == 1), 16], Sel_Crit[which(kmeans$cluster == 3), 16], alternative = "two.sided")$p.value, 3), sep = ""), paste("p(2&3) = ", signif(t.test(Sel_Crit[which(kmeans$cluster == 2), 16], Sel_Crit[which(kmeans$cluster == 3), 16], alternative = "two.sided")$p.value, 3), sep = "")), pch = 16, col = c("blue", NA, "red", NA, "orange", NA, NA, NA, NA, NA))
...
legend("topright", 
       inset = c(-0.475,0), 
       legend = c(paste("Cluster 1 (n = ", length(which(kmeans$cluster == 1)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit[which(kmeans$cluster == 1), 16], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit[which(kmeans$cluster == 1), 16], na.rm = T)/sqrt(length(which(kmeans$cluster == 1))), 2), ")", sep = ""), paste("Cluster 2 (n = ", length(which(kmeans$cluster == 2)), ")", sep = ""), paste("(mu = ", round(mean(Sel_Crit[which(kmeans$cluster == 2), 16], na.rm = T), 2),",", " sd = ", round(sd(Sel_Crit[which(kmeans$cluster == 2), 16], na.rm = T)/sqrt(length(which(kmeans$cluster == 2))), 2), ")", sep = ""), "", paste("p(1&2) = ", signif(t.test(Sel_Crit[which(kmeans$cluster == 1), 16], Sel_Crit[which(kmeans$cluster == 2), 16], alternative = "two.sided")$p.value, 3), sep = "")), 
       pch = 16, 
...
draw.triple.venn(
  area1 = length(which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial")),   # Our identified RDPs
  ...
  n12 = length(which(rownames(Table_HS_RBP) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial"), ]))), 
  n23 = length(which(Table_HS_RBP$R.DeeP_HeLa == "X")),
  n13 = length(which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial"), ]))), 
  n123 = length(which(which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial"), ])) %in% which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(Table_HS_RBP)))),
  fill = c("dodgerblue", "darkorange", "red"),
  lwd = 1,
  category = c(paste("Our RDPs (", length(which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial")), ")", sep = ""), paste("RBP2GO RBPs (", nrow(Table_HS_RBP), ")", sep =""), paste("R-Deep (", length(which(Table_HS_RBP$R.DeeP_HeLa == "X")), ")", sep = "")),
  fontfamily = "plain",
...
draw.triple.venn(
  area1 = length(which(Sel_Crit[, 15] == "no")),   # Our identified non RDPs
  area2 = nrow(Table_HS_RBP),  # RBPs in RBP2GO-Databank
  area3 = length(which(Table_HS_RBP$R.DeeP_HeLa == "X")),   # RDPs from the R-Deep-Databank
  n12 = length(which(rownames(Table_HS_RBP) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "no"), ]))), 
  n23 = length(which(Table_HS_RBP$R.DeeP_HeLa == "X")),
  n13 = length(which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "no"), ]))), 
  n123 = length(which(which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "no"), ])) %in% which(rownames(Table_HS_RBP[which(Table_HS_RBP$R.DeeP_HeLa == "X"),]) %in% rownames(Table_HS_RBP)))),
  fill = c("dodgerblue", "darkorange", "red"),
  lwd = 1,
  category = c(paste("Our non RDPs (", length(which(Sel_Crit[, 15] == "no")), ")", sep = ""), paste("RBP2GO RBPs (", nrow(Table_HS_RBP), ")", sep =""), paste("R-Deep (", length(which(Table_HS_RBP$R.DeeP_HeLa == "X")), ")", sep = "")),
  fontfamily = "plain",
  cat.fontfamily = "plain",
  cat.fontface = "bold",
  cat.dist = c(0.09, 0.085, 0.08),
...
par(mar=c(1, 3, 1, 7.5))
barplot(matrix(c(
  length(which(rownames(Sel_Crit[which(Sel_Crit[, 15] == "no"), ]) %in% rownames(Table_HS_Non_RBP))) + length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "no"), ]))),
  length(which(rownames(Sel_Crit[which(Sel_Crit[, 15] == "no"), ]) %in% rownames(Table_HS_RBP))),
  length(which(rownames(Sel_Crit[which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial"), ]) %in% rownames(Table_HS_RBP))),
  length(which(rownames(Sel_Crit[which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial"), ]) %in% rownames(Table_HS_Non_RBP))) + length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial"), ]))),
  rep(NA, 4)),
  nrow = 4),
  axes = F,
  col = c("red", "orange", "yellow", "green"), 
  legend.text = c(paste("Non RDPs (", length(which(rownames(Sel_Crit[which(Sel_Crit[, 15] == "no"), ]) %in% rownames(Table_HS_Non_RBP))) + length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "no"), ]))), ")", sep = ""), 
                  paste("RDPs (only RBP2GO) (", length(which(rownames(Sel_Crit[which(Sel_Crit[, 15] == "no"), ]) %in% rownames(Table_HS_RBP))), ")", sep = ""), 
                  paste("RDPS (", length(which(rownames(Sel_Crit[which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial"), ]) %in% rownames(Table_HS_RBP))), ")", sep = ""), 
                  paste("New RDPs (", length(which(rownames(Sel_Crit[which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial"), ]) %in% rownames(Table_HS_Non_RBP))) + length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(Sel_Crit[which(Sel_Crit[, 15] == "yes" | Sel_Crit[, 15] == "partial"), ]))), ")", sep = "")),
  )
```
...
```{r gene plotter}
Gene_Plotter(c("TCRG1_HUMAN"))
Sel_Crit[which(rownames(Sel_Crit) == "TCRG1_HUMAN"), c(1, 10, 12)]
```
