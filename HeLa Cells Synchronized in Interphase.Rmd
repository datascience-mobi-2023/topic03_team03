...
plot(1:25, Mean_Ctrl_Norm_100_Smooth[which(rownames(Mean_Ctrl)==gene.name),],type="l",lwd=4,col="green",xlab="Protein Fractions",ylab="Relative Protein Amount",main=paste(paste(gene.name, "(Normalized and Smoothed)")), ylim=c(0,max(max(Mean_Ctrl_Norm_100_Smooth[which(rownames(Mean_Ctrl)==gene.name),]), max(Mean_RNase_Norm_100_Smooth[which(rownames(Mean_Ctrl)==gene.name),]))+3))

abline(v=Mean_Ctrl_Gbl_Max[which(rownames(Mean_Ctrl)==gene.name),],lwd=3,col="green",lty=1)
abline(v=Mean_Ctrl_Loc_Max[which(rownames(Mean_Ctrl)==gene.name),],lwd=3,col="palegreen",lty=4)

lines(1:25, Mean_RNase_Norm_100_Smooth[which(rownames(Mean_RNase)==gene.name),], type="l",lwd=4,col="red")

abline(v=Mean_RNase_Gbl_Max[which(rownames(Mean_RNase)==gene.name),],lwd=3,col="red",lty=2)
abline(v=Mean_RNase_Loc_Max[which(rownames(Mean_RNase)==gene.name),],lwd=3,col="tomato",lty=3)

grid()

par(xpd=T)

legend("topright",inset = c(-0.25,0), legend=c("Control","RNase"),col=c("green","red"),bg="white",lwd=2)
