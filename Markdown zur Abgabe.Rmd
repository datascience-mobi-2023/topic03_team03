...
## 2.1 Referenced Data
The protocol for an R-DeeP-Screen was performed on HeLa cells synchronized in interphase (Caudron-Herger *et al.*, 2020) and the resulting protein amounts per fraction were stored in the dataframe *RDeep_HeLa_Interphase*.
...
Smoothing was achieved by calculating the mean value for each fraction through its defined neighborhood of three with the following formula (applies to all fractions between 2 and 24): \[y = \frac{f(x-1)+f(x)+f(x-1)}{3}\]    
...
In order to be able to compare the different protein amounts, the smoothed dataframes were normalized to 100. This means the sum of the mass spectrometry data in the different fractions for one protein added up to 100: \[y = \frac{f(x)}{\sum_{x=1}^{25}f(x)} *100\]     
...
that is lost under the global peak of Control after RNase treatment,
and that is gained under the global peak of RNase after RNase treatment,  
...
$\Delta\mu = \mu(Control)-\mu(RNase)> +1$: right shift; $\Delta\mu = \mu(Control)-\mu(RNase)< -1$: left shift; $-1 \leq\Delta\mu = \mu(Control)-\mu(RNase)\leq +1$: no shift;$\mu(Control)>24 \lor \mu(RNase)>24$: precipitated  
...
plot(Ctrl_Gauss_Fit[, 1], RNase_Gauss_Fit[, 1], pch = 16, col = col, main = "Shifting Behavior", xlab = "Control Gbl Peak Position", ylab = "RNase Gbl Peak Position")
...
Application of the selection criteria resulted in 539 RDPs, 274 partial RDPs and 5177 non RDPs (*Fig. 5 (left)*, p. 9). 
...
```
\footnotesize  
**Figure A.1: PCA with Shifting Behavior.** The first two principle components of the PCA applied to all selection criteria (left) and of the PCA applied to the optimal selection criteria are plotted against each other. The shifting classification of the proteins is shown by different colors.\normalsize
