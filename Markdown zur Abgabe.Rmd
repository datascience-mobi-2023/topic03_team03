...
## 2.2 Data Cleanup
The original dataset *RDeep_HeLa_Interphase* was split into two separate data frames containing the values for all RNase fractions and Control fractions respectively. These dataframes were named *RNase_RDeep* and accordingly *Ctrl_RDeep*. The following steps were performed on those two dataframes.
...
To test the reproducibility of the dataset the pearson correlation was performed. In seven cases, one of the three replicates contained only zeros. These values were replaced in *RNase_RDeep* and *Ctrl_RDeep* with NA as determining the pearson correlation would have been impossible. In five rows, two out of the three replicates contained only zeros. These genes were removed completely from the dataframes.
The pearson correlation was computed via the following formula: $r = \frac{1}{N-1} \sum_{i = 1}^{N}{\frac{x_i - \bar{x}}{s_x} \frac{y_i - \bar{y}}{s_y}}$
...
To check the effectiveness of the previously undertaken cleanup steps and determine the following steps, the data was visualized in its various stages. Two new dataframes were created which incorporated the mean into normalization called *Ctrl_Mean_Norm100* and *RNase_Mean_Norm100*. For example, the gene CTCF_HUMAN can be plotted with and without the normalization:

```{r Data Exploration, echo=FALSE, out.width = "50%"}
...
### not normalized
plot(1:25, Ctrl_Mean[which(rownames(Ctrl_RDeep)==c("CTCF_HUMAN")),],type="l",lwd=3,col="green",xlab="Fractions",ylab="Relative Protein Amount",main="CTCF_HUMAN Not Normalized")
...
Furthermore, *partially* RNA-dependent proteins were defined as proteins that either satisfy the first, fourth and second or third criterion for RNA dependence (1. & 4. & (2. | 3.)) or that satisfy the following criteria:
...
K means clustering was applied to the principal component analysis for comparison of these clusters with the identified RNA-dependent proteins. An elbow plot was used to identify the optimal number of clusters.  

### 2.6.2 linear Regression    
Linear regression was applied to the loss of protein amount under the Gaussian curve of the global peak for the control sample and the gain of protein amount under the Gaussian curve of the global peak for the RNase sample. This aimes at identifying proteins with a partial shift.
...
```{r smoothed and normalized, echo=FALSE, out.width = "50%"}
...
```{r Principal components, echo=False, out.extra="50%"}
...
```{r comparision with RBP2Go Database, echo =False, out.width="50%"}
...
```{r Vergleich unserer Werte mit dem RBP2Go score, echo = False, out.width="50%"}
...
## 3.7 Venn Diagram & Bar plot

```{r Venn diagramm RBPs, echo=False, out.width="50%"}
library("VennDiagram")
...
## 3.8 Bar plot with wierd analysis?
