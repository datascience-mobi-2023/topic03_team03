...
Finally, using linear regression we investigated whether the difference in protein amount between the control and RNase samples at the fraction position of the control global peak (Binding Affinity) could predict the full width at half maximum of the Gaussian curve fitted to the control global peak (Binding Specificity). Note that only RDPs where investigated. 
...
Performing a linear regression to test whether or not binding affinity is a good predictor of binding specificity returns a rather low coefficient of determination (R² = 0.29). It is noticeable that the calculated correlation between binding affinity and specificity is inverse, meaning that on average the full width at half maximum decreases with an increasing difference between the protein amounts of control and RNase samples at the global control peak position.
