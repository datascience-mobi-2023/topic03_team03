...
```{r setup, include=FALSE}
RDeeP_HeLa_Interphase = read.csv("https://www.dropbox.com/s/bn74lmbsqj6gzk7/RDeep_HeLa_Interphase.csv?dl=1", header=TRUE, row.names = 1, sep = ",")
...
##### 1.3 Normalize Rows to 100

This will be done for the Control and RNase Datasets separately:

```{r cleanup, include=FALSE}
Rep1 = Ctrl_RDeep[, seq(1,73,3)]

Rep1_Smooth = 
  as.data.frame(
cbind((Rep1[,1]+Rep1[,2])/2,
      sapply(seq(0,22,1), function(i) {apply(Rep1[,c(1,2,3)+c(i,i,i)], 1, function(x) {mean(x)})}), 
      (Rep1[,24]+Rep1[,25])/2)
)

colnames(Rep1_Smooth) = colnames(Rep1)

Rep1_Smooth_Norm = as.data.frame(sapply(Rep1_Smooth, function(i){(i/apply(Rep1_Smooth,1, function(x) {sum(x, na.rm = T)}))*100}))


Rep2 = Ctrl_RDeep[, seq(2,74,3)]

Rep2_Smooth = 
  as.data.frame(
cbind((Rep2[,1]+Rep2[,2])/2,
      sapply(seq(0,22,1), function(i) {apply(Rep2[,c(1,2,3)+c(i,i,i)], 1, function(x) {mean(x)})}), 
      (Rep2[,24]+Rep2[,25])/2)
)

colnames(Rep2_Smooth) = colnames(Rep2)

Rep2_Smooth_Norm = as.data.frame(sapply(Rep2_Smooth, function(i){(i/apply(Rep2_Smooth,1, function(x) {sum(x, na.rm = T)}))*100}))


Rep3 = Ctrl_RDeep[, seq(3,75,3)]

Rep3_Smooth = 
  as.data.frame(
cbind((Rep3[,1]+Rep3[,2])/2,
      sapply(seq(0,22,1), function(i) {apply(Rep3[,c(1,2,3)+c(i,i,i)], 1, function(x) {mean(x)})}), 
      (Rep3[,24]+Rep3[,25])/2)
)

colnames(Rep3_Smooth) = colnames(Rep3)

Rep3_Smooth_Norm = as.data.frame(sapply(Rep3_Smooth, function(i){(i/apply(Rep3_Smooth,1, function(x) {sum(x, na.rm = T)}))*100}))


Ctrl_Norm100 = cbind(Rep1_Smooth_Norm, Rep2_Smooth_Norm, Rep3_Smooth_Norm)

Ctrl_Norm100 = Ctrl_Norm100[, as.vector(sapply(seq(0,24,1), function(n) {c(1,26,51) + c(n,n,n)}))]



Rep1 = RNase_RDeep[, seq(1,73,3)]
...
RNase_Norm100 = cbind(Rep1_Smooth_Norm, Rep2_Smooth_Norm, Rep3_Smooth_Norm)

RNase_Norm100 = RNase_Norm100[, as.vector(sapply(seq(0,24,1), function(n) {c(1,26,51) + c(n,n,n)}))]

rm(Rep1, Rep1_Smooth, Rep1_Smooth_Norm, Rep2, Rep2_Smooth, Rep2_Smooth_Norm, Rep3, Rep3_Smooth, Rep3_Smooth_Norm)
```


##### 1.4 Pearson Correlation (Reproducibility)
...
Ctrl_Norm100 <- Ctrl_Norm100[!(rownames(Ctrl_Norm100) %in% c("BHMT2_HUMAN", "MAP9_HUMAN", "ECT2_HUMAN", "GP161_HUMAN", "PATZ1_HUMAN")),]

RNase_Norm100 <- RNase_Norm100[!(rownames(RNase_Norm100) %in% c("BHMT2_HUMAN", "MAP9_HUMAN", "ECT2_HUMAN", "GP161_HUMAN", "PATZ1_HUMAN")),]
...
Ctrl_Norm100[which(rownames(Ctrl_Norm100) == "BMX_HUMAN"), c(sapply(seq(0,72,3), function(k) {1 + k}))] = NA

Ctrl_Norm100[which(rownames(Ctrl_Norm100) == "DCAF7_HUMAN"), c(sapply(seq(0,72,3), function(k) {1 + k}))] = NA

Ctrl_Norm100[which(rownames(Ctrl_Norm100) == "TSC2_HUMAN"), c(sapply(seq(0,72,3), function(k) {1 + k}))] = NA

RNase_Norm100[which(rownames(Ctrl_Norm100) == "CCDC9_HUMAN"), c(sapply(seq(0,72,3), function(k) {2 + k}))] = NA

RNase_Norm100[which(rownames(Ctrl_Norm100) == "CQ051_HUMAN"), c(sapply(seq(0,72,3), function(k) {2 + k}))] = NA

RNase_Norm100[which(rownames(Ctrl_Norm100) == "HEM0_HUMAN"), c(sapply(seq(0,72,3), function(k) {3 + k}))] = NA

RNase_Norm100[which(rownames(Ctrl_Norm100) == "TSN3_HUMAN"), c(sapply(seq(0,72,3), function(k) {3 + k}))] = NA
...
Ctrl_Norm100 <- Ctrl_Norm100[- Gene.Pos,]

RNase_Norm100 <- RNase_Norm100[- Gene.Pos,]
...
Ctrl_Norm100[which(Ctrl_Cor_Rep[, 1] == Ctrl_Cor_Rep[, 3] & rowSums(Ctrl_Cor_Rep) == 1), c(sapply(seq(0,72,3), function(k) {1 + k}))] = NA

Ctrl_Norm100[which(Ctrl_Cor_Rep[, 1] == Ctrl_Cor_Rep[, 2] & rowSums(Ctrl_Cor_Rep) == 1), c(sapply(seq(0,72,3), function(k) {2 + k}))] = NA

Ctrl_Norm100[which(Ctrl_Cor_Rep[, 2] == Ctrl_Cor_Rep[, 3] & rowSums(Ctrl_Cor_Rep) == 1), c(sapply(seq(0,72,3), function(k) {3 + k}))] = NA


RNase_Norm100[which(RNase_Cor_Rep[, 1] == RNase_Cor_Rep[, 3] & rowSums(RNase_Cor_Rep) == 1), c(sapply(seq(0,72,3), function(k) {1 + k}))] = NA

RNase_Norm100[which(RNase_Cor_Rep[, 1] == RNase_Cor_Rep[, 2] & rowSums(RNase_Cor_Rep) == 1), c(sapply(seq(0,72,3), function(k) {2 + k}))] = NA

RNase_Norm100[which(RNase_Cor_Rep[, 2] == RNase_Cor_Rep[, 3] & rowSums(RNase_Cor_Rep) == 1), c(sapply(seq(0,72,3), function(k) {3 + k}))] = NA
...
Ctrl_Norm100[which(Ctrl_Cor_Rep[, 1] == 0 & rowSums(Ctrl_Cor_Rep) == 2 & Ctrl_Cor[, 2] > Ctrl_Cor[, 3]), c(sapply(seq(0,72,3), function(k) {1 + k}))] = NA

Ctrl_Norm100[which(Ctrl_Cor_Rep[, 1] == 0 & rowSums(Ctrl_Cor_Rep) == 2 & Ctrl_Cor[, 2] < Ctrl_Cor[, 3]), c(sapply(seq(0,72,3), function(k) {2 + k}))] = NA


Ctrl_Norm100[which(Ctrl_Cor_Rep[, 2] == 0 & rowSums(Ctrl_Cor_Rep) == 2 & Ctrl_Cor[, 1] > Ctrl_Cor[, 3]), c(sapply(seq(0,72,3), function(k) {3 + k}))] = NA

Ctrl_Norm100[which(Ctrl_Cor_Rep[, 2] == 0 & rowSums(Ctrl_Cor_Rep) == 2 & Ctrl_Cor[, 1] < Ctrl_Cor[, 3]), c(sapply(seq(0,72,3), function(k) {2 + k}))] = NA


Ctrl_Norm100[which(Ctrl_Cor_Rep[, 3] == 0 & rowSums(Ctrl_Cor_Rep) == 2 & Ctrl_Cor[, 1] > Ctrl_Cor[, 2]), c(sapply(seq(0,72,3), function(k) {3 + k}))] = NA

Ctrl_Norm100[which(Ctrl_Cor_Rep[, 3] == 0 & rowSums(Ctrl_Cor_Rep) == 2 & Ctrl_Cor[, 1] < Ctrl_Cor[, 2]), c(sapply(seq(0,72,3), function(k) {1 + k}))] = NA



RNase_Norm100[which(RNase_Cor_Rep[, 1] == 0 & rowSums(RNase_Cor_Rep) == 2 & RNase_Cor[, 2] > RNase_Cor[, 3]), c(sapply(seq(0,72,3), function(k) {1 + k}))] = NA

RNase_Norm100[which(RNase_Cor_Rep[, 1] == 0 & rowSums(RNase_Cor_Rep) == 2 & RNase_Cor[, 2] < RNase_Cor[, 3]), c(sapply(seq(0,72,3), function(k) {2 + k}))] = NA


RNase_Norm100[which(RNase_Cor_Rep[, 2] == 0 & rowSums(RNase_Cor_Rep) == 2 & RNase_Cor[, 1] > RNase_Cor[, 3]), c(sapply(seq(0,72,3), function(k) {3 + k}))] = NA

RNase_Norm100[which(RNase_Cor_Rep[, 2] == 0 & rowSums(RNase_Cor_Rep) == 2 & RNase_Cor[, 1] < RNase_Cor[, 3]), c(sapply(seq(0,72,3), function(k) {2 + k}))] = NA


RNase_Norm100[which(RNase_Cor_Rep[, 3] == 0 & rowSums(RNase_Cor_Rep) == 2 & RNase_Cor[, 1] > RNase_Cor[, 2]), c(sapply(seq(0,72,3), function(k) {3 + k}))] = NA

RNase_Norm100[which(RNase_Cor_Rep[, 3] == 0 & rowSums(RNase_Cor_Rep) == 2 & RNase_Cor[, 1] < RNase_Cor[, 2]), c(sapply(seq(0,72,3), function(k) {1 + k}))] = NA
...
par(xpd=T)
legend("topright", inset = c(-0.39,0), legend = c(paste("Cluster 1 (", length(which(kmeans$cluster == 1)), ")", sep = ""), paste("Cluster 2 (", length(which(kmeans$cluster == 2)), ")", sep = ""), paste("Cluster 3 (", length(which(kmeans$cluster == 3)), ")", sep = "")), pch = 16, col = c("blue", "red", "orange"))
...
##### 5.2 Comparison with the RBP2GO Database
...
###### 5.2.2 RBP2GO-Score: Confetti Plots
...
###### 5.2.3 RBP2GO-Score: Opacity Plots

Maybe we have more success if we do not use different colors but give genes with a higher RBP2GO-Score more opacity and color regions with higher points density a different color:
...
abline(h = 0, lty =3, col = "grey")
abline(v = 0, lty = 3, col = "grey")
points(pca$x[, 1], pca$x[, 2], pch = 16, col = densCols(pca$x[, 1], pca$x[, 2], colramp = colorRampPalette(c(alpha("red", 0), "red"), alpha = T)))
...
###### 5.2.4 RBP2GO-Score: t-Test

We will calculate the **mean RBP2GO score** for each of our RDP groups (RDPs, partial RDPs and non RDPs) and using a two-sided Welch two-sample t-test we can determine if the differences between these means is significant (H0: *The mean values are not significantly different from each other*, H1: *The mean values are significantly different from each other*):

```{r}
# ---> Mittelwert des RBP2GO scores für RDPs
mean(Sel_Crit[which(Sel_Crit[, 26] == "yes"), 27], na.rm = T)

# ---> Mittelwert des RBP2GO scores für partial RDPs
mean(Sel_Crit[which(Sel_Crit[, 26] == "partial"), 27], na.rm = T)

# ---> Mittelwert des RBP2GO scores für non RDPs
mean(Sel_Crit[which(Sel_Crit[, 26] == "no"), 27], na.rm = T)


# ---> Standardfehler des RBP2GO scores für RDPs
sd(Sel_Crit[which(Sel_Crit[, 26] == "yes"), 27], na.rm = T)/sqrt(length(Sel_Crit[which(Sel_Crit[, 26] == "yes"), 27]))

# ---> Standardfehler des RBP2GO scores für partial RDPs
sd(Sel_Crit[which(Sel_Crit[, 26] == "partial"), 27], na.rm = T)/sqrt(length(Sel_Crit[which(Sel_Crit[, 26] == "partial"), 27]))

# ---> Standardfehler des RBP2GO scores für non RDPs
sd(Sel_Crit[which(Sel_Crit[, 26] == "no"), 27], na.rm = T)/sqrt(length(Sel_Crit[which(Sel_Crit[, 26] == "no"), 27]))

# ---> p-value of t.test between RDPs and non RDPs
t.test(Sel_Crit[which(Sel_Crit[, 26] == "yes"), 27], Sel_Crit[which(Sel_Crit[, 26] == "no"), 27], alternative = "two.sided")$p.value

# ---> p-value of t.test between RDPs and partial RDPs
t.test(Sel_Crit[which(Sel_Crit[, 26] == "yes"), 27], Sel_Crit[which(Sel_Crit[, 26] == "partial"), 27], alternative = "two.sided")$p.value

# ---> p-value of t.test between partial RDPs and non RDPs
t.test(Sel_Crit[which(Sel_Crit[, 26] == "partial"), 27], Sel_Crit[which(Sel_Crit[, 26] == "no"), 27], alternative = "two.sided")$p.value
```

###### 5.2.5 Barplots (& Venn-Diagrams)
...
# ---> Barplot 2: Vergleich unserer RDPs (inklusive Weird Analysis) mit RBPs von der RBP2GO Datenbank
par(mar=c(1, 3, 3, 7.5))
barplot(
  matrix(
  c(
    length(which(rownames(Sel_Crit[which(Sel_Crit[, 26] == "no"), ]) %in% rownames(Table_HS_Non_RBP))) +
    length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(Sel_Crit[which(Sel_Crit[, 26] == "no"), ]))) +
    length(which(rownames(Weird_Sel_Crit[which(Weird_Sel_Crit[, 15] == "no"), ]) %in% rownames(Table_HS_Non_RBP))) +
    length(which(rownames(Weird_Ctrl_RDeep[which(!(rownames(Weird_Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Weird_Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(Weird_Sel_Crit[which(Weird_Sel_Crit[, 15] == "no"), ]))), 
    # RED: which protein are identified as non RDPs in our analysis and in RBP2GO? + which proteins from our dataset are not included in RBP2GO? (for both normal and weird analysis)
  
    length(which(rownames(Sel_Crit[which(Sel_Crit[, 26] == "no"), ]) %in% rownames(Table_HS_RBP))) +
    length(which(rownames(Weird_Sel_Crit[which(Weird_Sel_Crit[, 15] == "no"), ]) %in% rownames(Table_HS_RBP))), 
    # ORANGE: which of our non RDPs are in the RBP dataset from RBP2GO? (for both normal and weird analysis)
  
    length(which(rownames(Sel_Crit[which(Sel_Crit[, 26] == "yes" | Sel_Crit[, 26] == "partial"), ]) %in% rownames(Table_HS_RBP))) + 
    length(which(rownames(Weird_Sel_Crit[which(Weird_Sel_Crit[, 15] == "yes" | Weird_Sel_Crit[, 15] == "partial"), ]) %in% rownames(Table_HS_RBP))), 
    # YELLOW: which of our RDPs or partial RDPs are also listed in the RBP2GO databank? (for both normal and weird analysis)
  
    length(which(rownames(Sel_Crit[which(Sel_Crit[, 26] == "yes" | Sel_Crit[, 26] == "partial"), ]) %in% rownames(Table_HS_Non_RBP))) + 
    length(which(rownames(Ctrl_RDeep[which(!(rownames(Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(Sel_Crit[which(Sel_Crit[, 26] == "yes" | Sel_Crit[, 26] == "partial"), ]))) + 
    length(which(rownames(Weird_Sel_Crit[which(Weird_Sel_Crit[, 15] == "yes" | Weird_Sel_Crit[, 15] == "partial"), ]) %in% rownames(Table_HS_Non_RBP))) + 
    length(which(rownames(Weird_Ctrl_RDeep[which(!(rownames(Weird_Ctrl_RDeep) %in% rownames(Table_HS_RBP)) & !(rownames(Weird_Ctrl_RDeep) %in% rownames(Table_HS_Non_RBP))), ]) %in% rownames(Weird_Sel_Crit[which(Weird_Sel_Crit[, 15] == "yes" | Weird_Sel_Crit[, 15] == "partial"), ]))), 
    # GREEN: which of our RDPs are not listed in the RBP2GO Databank?
  ...
  axes = F,
  col = c("red", "orange", "yellow", "green"), 
  main = "Comparison with RBP2GO (including Weird)"
)
legend("topright", 
       fill = c("green", "yellow", "orange", "red"), 
       legend = c(
...
We will now test how well of a predictor **binding affinity** is to **binding specificity** for our RDPs. We define binding affinity as the protein amount difference between the Ctrl and RNase samples at the fraction position of the global Ctrl peak and binding specificity as the full width at half maximum at the global Ctrl peak.
...
par(xpd = T)
legend("topright", inset = c(-0.4, 0), legend = c("RDP", "Regression line", paste("y = ", round(fit$coefficients[2], 2), "x + ", round(fit$coefficients[1], 2), sep = ""), paste("R² = ", round(summary(fit)$r.squared, 2), sep = "")), pch = c(16, NA, NA, NA), col = c("blue", "red", NA, NA), lwd = c(NA, 3, NA, NA))
...
curve(fit$coefficients[1] + fit$coefficients[2]*x, add = T, lwd = 3, col = "red")
par(xpd = T)
legend("topright", inset = c(-0.4, 0), legend = c("RDP", "partial RDP", "non RDP", "Regression line", paste("y = ", round(fit$coefficients[2], 3), "x + ", round(fit$coefficients[1], 2), sep = ""), paste("R² = ", round(summary(fit)$r.squared, 2), sep = "")), pch = c(16, 16, 16, NA, NA, NA), col = c("blue", "orange", "grey", "red", NA, NA), lwd = c(NA, NA, NA, 3, NA, NA))
...
plot = scatterplot3d(Sel_Crit[, 27], Sel_Crit[, 1], Sel_Crit[, 11], pch = NA, box = F, grid = F, angle = 45)
source('http://www.sthda.com/sthda/RDoc/functions/addgrids3d.r')
addgrids3d(Sel_Crit[, 27], Sel_Crit[, 1], Sel_Crit[, 11], grid = c("xy", "xz", "yz"), angle = 45)
plot$points3d(Sel_Crit[, 27], Sel_Crit[, 1], Sel_Crit[, 11], pch = 16, col = col)

fit <- lm(Sel_Crit[, 27] ~ Sel_Crit[, 1] + Sel_Crit[, 11])
plot$plane3d(fit, draw_polygon = T, draw_lines = T, col = c("black"))
...
Gene_Plotter(gene.name, col = c("green", "red", "dodgerblue", "darkorange"), lwd = 3, point.size = 1.5, main = TRUE, cus.main = NULL, ylab = "Relative Protein Amount", xlab = "Protein Fractions", fs = c(1.2, 1, 1), Leg.Size = 1, Leg.Pos = c(0, 0), mar = c(5, 4, 4, 7), Legend = TRUE, Ctrl = TRUE, RNase = TRUE, Gbl.Max = TRUE, Loc.Max = TRUE, Gauss = TRUE, conf = TRUE, grid = TRUE, Type = "Smooth and Norm")
...
But you can also specify the arguments to your liking:
  ...
  * **lwd**: line width of the protein distributions (does not change the line width in the legend)
  * **point.size**: size of the circles and triangles that show the local of the global and local peaks (does not change the point size in the legend)
  * **main**: logical argument (TRUE or FALSE) that specifies whether or not the main heading is shown
  * **cus.main**: customizable text option for the main heading of the plot 
  * **ylab**: text that is displayed on the y-axis
  * **xlab**: text that is displayed on the x-axis
  * **fs**: vector that specifies the font size for the elements of the plot (c(**main Heading, Axis Names, Axis Labels**))
  * **Leg.Size**: numeric that specifies the size of the legend
  * **Leg.Pos**: vector that specifies the position of the legend (c(x, y))
  * **mar**: vector that specifies the margin size (c(bottom, left, top, right))
  * **Legend**: logical argument (TRUE or FALSE) that specifies whether or not the legend is shown
...
Gene_Plotter <- function(gene.name, col = c("green", "red", "dodgerblue", "darkorange"), lwd = 3, point.size = 1.5, main = T, cus.main = NULL, ylab = "Relative Protein Amount", xlab = "Protein Fractions", fs = c(1.2, 1, 1), Leg.Size =  1, Leg.Pos = c(0, 0), mar = c(5, 4, 4, 7), Legend = T, Ctrl = T, RNase = T, Gbl.Max = T, Loc.Max = T, Gauss = T, conf = T, grid = T, Type = "Smooth and Norm" #, "Norm", "Smooth", "Raw", "Compare"
                          ) {
library(scales)
...
plot(1:25, rep(NA, 25),xlab=xlab,ylab=ylab,main = if(main == T) {if(length(cus.main) == 0) {paste(gene.name[j], "(Smoothed and Normalized)")} else {cus.main}}, ylim=c(0, 1.1 * max(
...
if(Ctrl == T) {    
lines(1:25, Ctrl_Mean_Smooth_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),], type="l", lwd=lwd, col = col[1])}
...
if(RNase == T) {
lines(1:25, RNase_Mean_Smooth_Norm100[which(rownames(RNase_Mean)==gene.name[j]),], type="l", lwd=lwd, col = col[2])}

if(Gbl.Max == T & Ctrl == T) {
points(Ctrl_Mean_Gbl_Max[which(rownames(Ctrl_Mean)==gene.name[j]), ], Ctrl_Mean_Smooth_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),unlist(Ctrl_Mean_Gbl_Max[which(rownames(Ctrl_Mean)==gene.name[j]),])], pch=21, col="black", bg=col[1], cex=point.size)}

if(Loc.Max == T & Ctrl == T & !is.null(unlist(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))]))) {
points(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))], Ctrl_Mean_Smooth_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]), which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))], pch=24, col="black", bg=col[1], cex=0.8*point.size)}

if(Loc.Max == T & RNase == T & !is.null(unlist(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))]))) {
points(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))], RNase_Mean_Smooth_Norm100[which(rownames(RNase_Mean)==gene.name[j]), which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))], pch=24, col="black", bg=col[2], cex=0.8*point.size)}

if(Gbl.Max == T & RNase == T) {
points(RNase_Mean_Gbl_Max[which(rownames(RNase_Mean)==gene.name[j]), ], RNase_Mean_Smooth_Norm100[which(rownames(RNase_Mean)==gene.name[j]),unlist(RNase_Mean_Gbl_Max[which(rownames(RNase_Mean)==gene.name[j]),])], pch=21, col="black", bg=col[2], cex=point.size)}

if(Gauss == T & Ctrl == T) {
curve(Ctrl_Gauss_Fit[which(rownames(Ctrl_Mean)==gene.name[j]), 3]*(1/(Ctrl_Gauss_Fit[which(rownames(Ctrl_Mean)==gene.name[j]), 2]*(sqrt(2*pi))))*exp(1)^(-(1/2)*((x-Ctrl_Gauss_Fit[which(rownames(Ctrl_Mean)==gene.name[j]), 1])/Ctrl_Gauss_Fit[which(rownames(Ctrl_Mean)==gene.name[j]), 2])^2), add = T, lwd=lwd, col = alpha(col[3], 0.5))
  
curve(Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),4]*(1/(Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),3]*(sqrt(2*pi))))*exp(1)^(-(1/2)*((x-Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),2])/Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),3])^2), add = T, lwd=lwd, col = alpha(col[3], 0.5))  
  }

if(Gauss == T & RNase == T) {
curve(RNase_Gauss_Fit[which(rownames(RNase_Mean)==gene.name[j]), 3]*(1/(RNase_Gauss_Fit[which(rownames(RNase_Mean)==gene.name[j]), 2]*(sqrt(2*pi))))*exp(1)^(-(1/2)*((x-RNase_Gauss_Fit[which(rownames(RNase_Mean)==gene.name[j]), 1])/RNase_Gauss_Fit[which(rownames(RNase_Mean)==gene.name[j]), 2])^2), add = T, lwd=lwd, col = alpha(col[4], 0.5))
  
  curve(Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),9]*(1/(Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),8]*(sqrt(2*pi))))*exp(1)^(-(1/2)*((x-Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),7])/Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),8])^2), add = T, lwd=lwd, col = alpha(col[4], 0.5))  
  }
if(Legend == T) {
par(xpd=T, cex = Leg.Size)

legend("topright",inset = c(-0.23,0) - Leg.Pos, 
...
plot(1:25, rep(NA, 25),xlab=xlab,ylab=ylab,main=if(main == T) {if(length(cus.main) == 0) {paste(gene.name[j], "(Normalized)")} else {cus.main}}, ylim=c(0, 1.1 * max(
...
if(Loc.Max == T & Ctrl == T & !is.null(unlist(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))]))) {
segments(unlist(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))]), -4.3, unlist(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))]), 110, col = col[1], lty = 3, lwd=lwd)}

if(Loc.Max == T & RNase == T & !is.null(unlist(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))]))) {
segments(unlist(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))]), -3.4, unlist(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))]), 110, col = col[2], lty = 3, lwd=lwd)}

if(Gbl.Max == T & Ctrl == T) {
segments(unlist(Ctrl_Mean_Gbl_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]), -4, unlist(Ctrl_Mean_Gbl_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]), 110, col = col[1], lty = 2, lwd=lwd)}

if(Ctrl == T) {
lines(1:25, Ctrl_Mean_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),], type="l", lwd=lwd, col = col[1])}

if(Gbl.Max == T & RNase == T) {
segments(unlist(RNase_Mean_Gbl_Max[which(rownames(RNase_Mean)==gene.name[j]),]), -3, unlist(RNase_Mean_Gbl_Max[which(rownames(RNase_Mean)==gene.name[j]),]), 110, col = col[2], lty = 2, lwd=lwd)}

if(RNase == T) {
lines(1:25, RNase_Mean_Norm100[which(rownames(RNase_Mean)==gene.name[j]),], type="l", lwd=lwd, col = col[2])}

if(Legend == T) {
par(xpd=T, cex = Leg.Size)

legend("topright",inset = c(-0.25,0) - Leg.Pos, 
...
plot(1:25, rep(NA, 25),xlab=xlab,ylab=ylab,main=if(main == T) {if(length(cus.main) == 0) {paste(gene.name[j], "(Smoothed)")} else {cus.main}}, ylim=c(0, 1.1 * max(
...
if(Ctrl == T) {  
lines(1:25, Ctrl_Mean_Smooth[which(rownames(Ctrl_Mean)==gene.name[j]),], type="l", lwd=lwd, col = col[1])}
...
if(RNase == T) {
lines(1:25, RNase_Mean_Smooth[which(rownames(RNase_Mean)==gene.name[j]),], type="l", lwd=lwd, col = col[2])}

if(Gbl.Max == T & Ctrl == T) {
points(Ctrl_Mean_Gbl_Max[which(rownames(Ctrl_Mean)==gene.name[j]), ], Ctrl_Mean_Smooth[which(rownames(Ctrl_Mean)==gene.name[j]),unlist(Ctrl_Mean_Gbl_Max[which(rownames(Ctrl_Mean)==gene.name[j]),])], pch=21, col="black", bg=col[1], cex=point.size)}

if(Loc.Max == T & Ctrl == T & !is.null(unlist(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))]))) {
points(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))], Ctrl_Mean_Smooth[which(rownames(Ctrl_Mean)==gene.name[j]), which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))], pch=24, col="black", bg=col[1], cex=0.8*point.size)}

if(Loc.Max == T & RNase == T & !is.null(unlist(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))]))) {
points(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))], RNase_Mean_Smooth[which(rownames(RNase_Mean)==gene.name[j]), which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))], pch=24, col="black", bg=col[2], cex=0.8*point.size)}

if(Gbl.Max == T & RNase == T) {
points(RNase_Mean_Gbl_Max[which(rownames(RNase_Mean)==gene.name[j]), ], RNase_Mean_Smooth[which(rownames(RNase_Mean)==gene.name[j]),unlist(RNase_Mean_Gbl_Max[which(rownames(RNase_Mean)==gene.name[j]),])], pch=21, col="black", bg=col[2], cex=point.size)}
...
plot(1:25, rep(NA, 25),xlab=xlab,ylab=ylab,main=if(main == T) {if(length(cus.main) == 0) {paste(gene.name[j], "(Raw Data)")} else {cus.main}}, ylim=c(0, 1.1 * max(
...
segments(unlist(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),which(!is.na(Ctrl_Mean_Loc_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]))]), - 0.07 * max(
  ...
  if(Ctrl == T) {max(Ctrl_Mean[which(rownames(Ctrl_Mean)==gene.name[j]),])}, 
  if(RNase == T) {max(RNase_Mean[which(rownames(Ctrl_Mean)==gene.name[j]),])}), col = col[1], lty = 3, lwd=lwd)}
...
segments(unlist(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),which(!is.na(RNase_Mean_Loc_Max[which(rownames(RNase_Mean)==gene.name[j]),]))]), -0.8 * max(
  ...
  if(Ctrl == T) {max(Ctrl_Mean[which(rownames(Ctrl_Mean)==gene.name[j]),])}, 
  if(RNase == T) {max(RNase_Mean[which(rownames(Ctrl_Mean)==gene.name[j]),])}), col = col[2], lty = 3, lwd=lwd)}
...
segments(unlist(Ctrl_Mean_Gbl_Max[which(rownames(Ctrl_Mean)==gene.name[j]),]), - 0.93 * max(
  ...
  if(Ctrl == T) {max(Ctrl_Mean[which(rownames(Ctrl_Mean)==gene.name[j]),])}, 
  if(RNase == T) {max(RNase_Mean[which(rownames(Ctrl_Mean)==gene.name[j]),])}), col = col[1], lty = 2, lwd=lwd)}

if(Ctrl == T) {  
lines(1:25, Ctrl_Mean[which(rownames(Ctrl_Mean)==gene.name[j]),], type="l", lwd=lwd, col = col[1])}
...
segments(unlist(RNase_Mean_Gbl_Max[which(rownames(RNase_Mean)==gene.name[j]),]), - 0.7 * max(
  ...
  if(Ctrl == T) {max(Ctrl_Mean[which(rownames(Ctrl_Mean)==gene.name[j]),])}, 
  if(RNase == T) {max(RNase_Mean[which(rownames(Ctrl_Mean)==gene.name[j]),])}), col = col[2], lty = 2, lwd=lwd)}

if(RNase == T) {
lines(1:25, RNase_Mean[which(rownames(RNase_Mean)==gene.name[j]),], type="l", lwd=lwd, col = col[2])}
...
par(mar=mar + c(0,0,0,3), xpd=F)
  
plot(1:25, rep(NA, 25),xlab=xlab,ylab=ylab,main=if(main == T) {if(length(cus.main) == 0) {paste(gene.name[j], "(Norm/Smooth v. Norm)")} else {cus.main}}, ylim=c(0, 1.1 * max(
  if(Ctrl == T) {max(Ctrl_Mean_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),])}, 
  if(RNase == T) {max(RNase_Mean_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),])}, 
  if(Ctrl == T) {max(Ctrl_Mean_Smooth_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),])}, 
  if(RNase == T) {max(RNase_Mean_Smooth_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),])})),
...
if(Ctrl == T) {
lines(1:25, Ctrl_Mean_Norm100[which(rownames(Ctrl_Mean)==gene.name[j]),], type="l", lwd=lwd, col = alpha(col[1], 0.35))
...
if(RNase == T) {
lines(1:25, RNase_Mean_Norm100[which(rownames(RNase_Mean)==gene.name[j]),], type="l", lwd=lwd, col = alpha(col[2], 0.35))
...
legend("topright",inset = c(-0.39,0) - Leg.Pos, 
...
```{r gene plotter}
Gene_Plotter("HNRPL_HUMAN", Type = c("Smooth and Norm"))
```
