...
In order to be able to compare different protein amounts, each protein of the smoothed datasets was normalized to a total sum of 100 across all fractions: \[y = \frac{f(x)}{\sum_{x=1}^{25}f(x)} *100\]     
...
The standard errors were scaled accordingly.
...
The identified RDPs were also compared directly to the RBP2GO database (*Fig. 6*): 2231 genes (37 % of all analyzed genes) which are categorized as non-RDPs are also listed as non-RBPs in the RBP2GO database. In addition, 631 (11 %) RDPs are included as RBPs in RBP2GO. Yet, nearly half of the analyzed genes (2946 (49 %)) are categorized as non-RDPs, even though RBP2GO lists them as RBPs. Moreover, 182 (3 %) RDPs are listed as non-RBPs in the RBP2GO database. These "new" RDPs include proteins which are known to be associated with RNA (e.g. *CDK13_HUMAN*, *Appendix Fig. A.5*) (Berro *et al.*, 2008).  

The differences between the presented results and the RBP2GO database are possibly the result of differences in the human cell lineages and experimental methods used and different stages of the cell cycle analyzed. Notably, only the proteome present in interphase was analyzed here, so suboptimal comparability with the RBP2GO database, which draws its data from many orthogonal experiments, should be expected.   

Lastly, a linear regression was performed to test whether molecular interaction parameters could be determined based on the mass spectrometry data. One could assume that an RDP which binds tightly to its RNA target also forms very specific structures and, therefore, has a high binding specificity. This relation was somewhat observed when applying linear regression modelling only to the identified RDPs (*Fig. 7, right*). A linear regression of all proteins, partial RDPs or non-RDPs result in a lower coefficient of correlation (*Fig. 7, left* and *Appendix Fig. A.6*). However, the experiment was not set up to investigate the kinetics of RNA-protein interactions. Other methods that have been developed especially to investigate the kinetics of RNA-protein interactions would be more suitable (see for instance Sharma *et al.*, 2021).  
...
#non-RDP:
Gene_Plotter("STAG2_HUMAN", cus.main = "STAG2_HUMAN (non-RDP)", Leg.Pos = c(0.05, 0), grid = F)
...
\footnotesize  
**Figure A.5: New RDP with an RNA Association.** *Cyclin-dependent kinase 13 (CDK13_HUMAN)* is involved in many key cellular RNA processes such as splicing and transcription (Berro *et al.*, 2008).\normalsize
...
```
\footnotesize  
**Figure A.6: Linear Regression for Partial RDPs and Non-RDPs.** Linear regression was applied to the binding affinity and the binding specificity for only partial RDPs (left) and for only non-RDPs (right). Both linear regressions have considerably smaller R² values compared to the linear regression of only RDPs.\normalsize
